...
uint32_t paging_new_address_space(void) {
     ...
     * ever called (Chapter 18's own task_create_process(), after this
     * task's kernel stack has already been kmalloc()'d), the kernel's
     * own directory may already hold real entries paging_init() never
     * built: 018_kmain.c's own 0xC0000000 demo mapping, and however
     * far the kheap (018_kheap.c) has grown by this point. Each entry
