 ...
 *
 * 031_kmain.c's own ring-3 demo triggers this by executing CLI at
 ...
 * ring 0, called from 031_isr128.asm's own INT 0x80 stub with
 * whatever the caller put in EAX/EBX forwarded as real cdecl
 * arguments. Now a real switch statement, exactly as 031_syscall.h's
 ...
 * call into 031_task.c's own existing task_yield()/task_exit() --
 ...
 * fixed task table.
 *
 * task_exit() never returns (see 031_task.c's own comment on it), so
 * the SYS_EXIT case below never falls through to 031_isr128.asm's own
