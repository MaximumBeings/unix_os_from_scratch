/* See 036_acord.h's own top-of-file comment for the full citation trail
 ...
 * other personal-auto-specific names in 036_acord.h: real per general
...
/* own stated subset (see 036_acord.h): no attributes, no whitespace  */
 ...
 * constant -- not off 036_acord.h's own ACORD_MAX_NAME_LEN, which
 ...
 * ACORD_MAX_TAG_LEN + 4 bytes is always enough (this file's own dedicated tag-name-sized constant, not 036_acord.h's ACORD_MAX_NAME_LEN, which sizes the unrelated surname/given-name fields -- see the fix note above make_wrapped_tag()). */
...
/* FormatCurrencyAmt > Amt -- the real OBSERVED pattern (036_acord.h),
...
uint32_t acord_build_request(const acord_request_t *r, uint8_t *out, uint32_t out_size) {
     ...
     * -- 036_insurance.c's own rating table needs the applicant's
