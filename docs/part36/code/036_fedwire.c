/* See 036_fedwire.h's own top-of-file comment for the full real citation
...
uint32_t fedwire_build_message(const fedwire_message_t *msg, uint8_t *out_buf, uint32_t out_buf_size) {
    ...
    {
        /* Real 32-bit division/modulo only (see 036_fedwire.h's own
...
int fedwire_parse_message(const uint8_t *buf, uint32_t len, fedwire_message_t *out_msg) {
    ...
    {
        /* Real 32-bit accumulation only -- see 036_fedwire.h's own
