#ifndef UNIX_OS_036_MULTIBOOT_H
#define UNIX_OS_036_MULTIBOOT_H
 ...
 * mboot_info_addr (the physical address this book's own 036_boot.asm
 ...
 * none. */
/* Returns the real "total_size" field itself -- the very first uint32_t
 * of the boot information structure, per the same cited spec passage
 * multiboot_find_mmap()'s own comment quotes. This chapter adds it so
 * kmain() can reserve the WHOLE structure's own physical bytes in the
 * frame allocator, the same way Chapter 17 already reserves the boot
 * MODULE's own bytes -- see kmain()'s own comment on the real
 * corruption this fixes. */
uint32_t multiboot_total_size(uint32_t mboot_info_addr);
 ...
 * all -- for instance, if 036_kmain.c's own real run were ever booted
