/* See 036_ach.h's own top-of-file comment for the full real citation of
...
/* Real alphanumeric field rule (cited in 036_ach.h): left-justified,
 * space-filled. */
...
uint8_t ach_compute_aba_check_digit(const uint8_t routing8[ACH_ROUTING_LEN]) {
     ...
     * 036_ach.h from Apache Commons Validator's own ABANumberCheckDigit
...
uint32_t ach_build_file(const ach_batch_t *batch, uint8_t *out_buf, uint32_t out_buf_size) {
     ...
     * real payroll batch already works (see 036_ach.h's own scope note).
     */
    for (uint32_t i = 0; i < batch->entry_count; i++) {
         ...
         * routing number, cited in 036_ach.h. */
     ...
     * cited in 036_ach.h. This chapter's own uint32_t entry_hash can
...
int ach_parse_file(const uint8_t *buf, uint32_t len, ach_batch_t *out_batch) {
     ...
     * 036_ach.h) means the real total record count is always a multiple
     ...
     * 036_fedwire.c's own fedwire_parse_message() precedent: fields are
