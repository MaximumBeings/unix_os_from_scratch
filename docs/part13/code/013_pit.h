#ifndef UNIX_OS_013_PIT_H
#define UNIX_OS_013_PIT_H
 ...
 * on the PIC (013_pic.c) so those interrupts can actually reach this
 ...
 * STI -- the same ordering 013_keyboard.h's keyboard_init() already
 * requires for IRQ1. */
...
/* The real C handler 013_irq0.asm's stub calls on every IRQ0. Since
 * this chapter, also drives this kernel's own preemptive scheduling
 * -- see 013_pit.c's own comment on why task_tick() is called only
