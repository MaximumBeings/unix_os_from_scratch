#ifndef UNIX_OS_032_TASK_H
#define UNIX_OS_032_TASK_H
 ...
 * process (032_task.c's own task_create_process()). Chosen well
 ...
 * carries forward unchanged, even though 032_kmain.c's own demo this
 ...
 * 032_kmain.c's own kmain() can call paging_translate_in() itself
 ...
 * into (032_multiboot.h's own multiboot_tag_module), before this
 ...
 * template function to one fixed virtual address, calls 032_elf.c's
 ...
 * fails to parse as a valid ELF32 executable (032_elf.c's own
 ...
 * task). New this chapter, so 032_semaphore.c can record which task
 ...
 * call on purpose: 032_semaphore.c needs this exact state transition
 ...
 * 032_semaphore.c's own comments for the real race this avoids). The
 ...
 * to that task. Called from 032_semaphore.c's semaphore_signal(),
...
/* Called from 032_pit.c's irq0_handler() on every single real IRQ0
