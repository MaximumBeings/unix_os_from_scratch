#ifndef UNIX_OS_011_ISR_HANDLERS_H
#define UNIX_OS_011_ISR_HANDLERS_H

#include <stdint.h>

/* The real C handler 011_isr0.asm's stub calls on a #DE fault. Nothing
 * in C calls it directly -- only the assembly stub does -- so this
 * declaration exists purely so 011_isr_handlers.c has a header to
 * define against, the same one-module-one-header shape every other
 * file in this book already follows. */
void isr0_handler(void);

/* The real C handler 011_isr14.asm's stub calls on a #PF fault, with
