#ifndef UNIX_OS_026_TASK_H
#define UNIX_OS_026_TASK_H
 ...
 * process (026_task.c's own task_create_process()). Chosen well
 ...
 * carries forward unchanged, even though 026_kmain.c's own demo this
 ...
 * 026_kmain.c's own kmain() can call paging_translate_in() itself
 ...
 * into (026_multiboot.h's own multiboot_tag_module), before this
 ...
 * template function to one fixed virtual address, calls 026_elf.c's
 ...
 * fails to parse as a valid ELF32 executable (026_elf.c's own
 ...
 * task). New this chapter, so 026_semaphore.c can record which task
 ...
 * call on purpose: 026_semaphore.c needs this exact state transition
 ...
 * 026_semaphore.c's own comments for the real race this avoids). The
 ...
 * to that task. Called from 026_semaphore.c's semaphore_signal(),
...
/* Called from 026_pit.c's irq0_handler() on every single real IRQ0
