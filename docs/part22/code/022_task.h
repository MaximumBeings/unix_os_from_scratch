#ifndef UNIX_OS_022_TASK_H
#define UNIX_OS_022_TASK_H
 ...
 * process (022_task.c's own task_create_process()). Chosen well
 ...
 * carries forward unchanged, even though 022_kmain.c's own demo this
 ...
 * 022_kmain.c's own kmain() can call paging_translate_in() itself
 ...
 * into (022_multiboot.h's own multiboot_tag_module), before this
 ...
 * template function to one fixed virtual address, calls 022_elf.c's
 ...
 * fails to parse as a valid ELF32 executable (022_elf.c's own
 ...
 * task). New this chapter, so 022_semaphore.c can record which task
 ...
 * call on purpose: 022_semaphore.c needs this exact state transition
 ...
 * 022_semaphore.c's own comments for the real race this avoids). The
 ...
 * to that task. Called from 022_semaphore.c's semaphore_signal(),
...
/* Called from 022_pit.c's irq0_handler() on every single real IRQ0
