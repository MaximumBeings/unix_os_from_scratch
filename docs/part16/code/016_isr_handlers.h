#ifndef UNIX_OS_016_ISR_HANDLERS_H
#define UNIX_OS_016_ISR_HANDLERS_H

#include <stdint.h>

/* The real C handler 016_isr0.asm's stub calls on a #DE fault. */
void isr0_handler(void);

/* The real C handler 016_isr13.asm's stub calls on a #GP fault, with
 ...
 * see 016_kmain.c's user_mode_entry() for what deliberately triggers
 * it. */
void isr13_handler(uint32_t error_code);

/* The real C handler 016_isr14.asm's stub calls on a #PF fault, with
 * the CPU's own real error code passed as its one argument. */
void isr14_handler(uint32_t error_code);

/* The real C handler 016_isr128.asm's stub calls on this chapter's
 * own INT 0x80 syscall gate: `syscall_num` is whatever the caller put
 * in EAX (016_syscall.h's own SYS_WRITE_STR/SYS_YIELD/SYS_EXIT), `arg`
 ...
 * own task_yield()/task_exit() (016_task.h) directly -- neither one
