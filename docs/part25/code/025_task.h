#ifndef UNIX_OS_025_TASK_H
#define UNIX_OS_025_TASK_H
 ...
 * process (025_task.c's own task_create_process()). Chosen well
 ...
 * carries forward unchanged, even though 025_kmain.c's own demo this
 ...
 * 025_kmain.c's own kmain() can call paging_translate_in() itself
 ...
 * into (025_multiboot.h's own multiboot_tag_module), before this
 ...
 * template function to one fixed virtual address, calls 025_elf.c's
 ...
 * fails to parse as a valid ELF32 executable (025_elf.c's own
 ...
 * task). New this chapter, so 025_semaphore.c can record which task
 ...
 * call on purpose: 025_semaphore.c needs this exact state transition
 ...
 * 025_semaphore.c's own comments for the real race this avoids). The
 ...
 * to that task. Called from 025_semaphore.c's semaphore_signal(),
...
/* Called from 025_pit.c's irq0_handler() on every single real IRQ0
