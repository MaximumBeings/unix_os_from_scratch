#ifndef UNIX_OS_027_TASK_H
#define UNIX_OS_027_TASK_H
 ...
 * process (027_task.c's own task_create_process()). Chosen well
 ...
 * carries forward unchanged, even though 027_kmain.c's own demo this
 ...
 * 027_kmain.c's own kmain() can call paging_translate_in() itself
 ...
 * into (027_multiboot.h's own multiboot_tag_module), before this
 ...
 * template function to one fixed virtual address, calls 027_elf.c's
 ...
 * fails to parse as a valid ELF32 executable (027_elf.c's own
 ...
 * task). New this chapter, so 027_semaphore.c can record which task
 ...
 * call on purpose: 027_semaphore.c needs this exact state transition
 ...
 * 027_semaphore.c's own comments for the real race this avoids). The
 ...
 * to that task. Called from 027_semaphore.c's semaphore_signal(),
...
/* Called from 027_pit.c's irq0_handler() on every single real IRQ0
