#ifndef UNIX_OS_023_TASK_H
#define UNIX_OS_023_TASK_H
 ...
 * process (023_task.c's own task_create_process()). Chosen well
 ...
 * carries forward unchanged, even though 023_kmain.c's own demo this
 ...
 * 023_kmain.c's own kmain() can call paging_translate_in() itself
 ...
 * into (023_multiboot.h's own multiboot_tag_module), before this
 ...
 * template function to one fixed virtual address, calls 023_elf.c's
 ...
 * fails to parse as a valid ELF32 executable (023_elf.c's own
 ...
 * task). New this chapter, so 023_semaphore.c can record which task
 ...
 * call on purpose: 023_semaphore.c needs this exact state transition
 ...
 * 023_semaphore.c's own comments for the real race this avoids). The
 ...
 * to that task. Called from 023_semaphore.c's semaphore_signal(),
...
/* Called from 023_pit.c's irq0_handler() on every single real IRQ0
