#ifndef UNIX_OS_035_PIT_H
#define UNIX_OS_035_PIT_H
 ...
 * on the PIC (035_pic.c) so those interrupts can actually reach this
 ...
 * STI -- the same ordering 035_keyboard.h's keyboard_init() already
 * requires for IRQ1. */
...
/* The real C handler 035_irq0.asm's stub calls on every IRQ0. Since
 * this chapter, also drives this kernel's own preemptive scheduling
 * -- see 035_pit.c's own comment on why task_tick() is called only
