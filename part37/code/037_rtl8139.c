 ...
 * limits -- see 037_rtl8139.h's own top-of-file comment for the full
 ...
 * 037_rtl8139.h's own top-of-file comment) -- kept purely so
 ...
 * 037_rtl8139.h's own comment on rtl8139_get_irq_count(). Chapter 26
...
int rtl8139_init(int enable_loopback) {
     ...
     * for it is still fixed at compile time (see 037_rtl8139.h's own
     * top-of-file comment). */
...
int rtl8139_send_queue(const uint8_t *frame, uint32_t len) {
     ...
     * 037_rtl8139.h's own top-of-file comment -- this driver's own
...
int rtl8139_receive_next_packet(uint8_t *out_buf, uint32_t *out_len) {
     ...
     * 037_rtl8139.h's own top-of-file comment. `rx_buf` is
    ...
    *out_len = copy_len;
    ...
    /* Real, cited CAPR advancement -- see 037_rtl8139.h's own
...
/* This chapter's own real interrupt handler -- see 037_rtl8139.h's
 ...
 * 037_irq11.asm's own stub, which -- exactly like every other IRQ
...
void irq11_handler(void) {
     ...
     * slave-PIC lines (8-15) -- cited directly, 037_pic.c's own
