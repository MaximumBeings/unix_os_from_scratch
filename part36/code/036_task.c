 ...
 * can now be BLOCKED (waiting on a semaphore, see 036_semaphore.c)
...
struct task {
    ...
    uint32_t kernel_stack_top; /* new this chapter: only meaningful when
                               ...
                               * (036_tss.c) immediately before switching
...
/* 036_usermode.asm's real ring 0 -> ring 3 transition -- Chapter 15's
...
int task_create_process(const void *template_start, uint32_t template_size) {
     ...
     * kmalloc() call may itself grow the kheap (036_kheap.c), and
    ...
    *(--sp) = 0;  /* edi */
     ...
     * physical frames -- 036_kmain.c's own kmain() proves this
...
int task_create_elf_process(uint32_t module_start, uint32_t module_end) {
    ...
    *(--sp) = 0;  /* edi */
     ...
     * kernel-chosen virtual address at all. 036_elf.c's own elf_load()
