 ...
 * elf_i386 -T 036_user_program.ld` invocation and its own dedicated
 * linker script -- fixing this program's own load address at 0xE9000000,
 * deliberately a different number than 036_task.h's own
 ...
 * real GRUB Multiboot2 boot MODULE (036_multiboot.h's own struct
 * multiboot_tag_module) alongside kernel.bin, not linked into it.
 * 036_elf.c's own elf_load() reads this file's real ELF header and
 ...
 * in this kernel's `readelf -h` output, is what 036_kmain.c's own
 ...
 * symbol 036_user_program.ld's own ENTRY(_start) directive names, and
