#ifndef UNIX_OS_021_TASK_H
#define UNIX_OS_021_TASK_H
 ...
 * process (021_task.c's own task_create_process()). Chosen well
 ...
 * carries forward unchanged, even though 021_kmain.c's own demo this
 ...
 * 021_kmain.c's own kmain() can call paging_translate_in() itself
 ...
 * into (021_multiboot.h's own multiboot_tag_module), before this
 ...
 * template function to one fixed virtual address, calls 021_elf.c's
 ...
 * fails to parse as a valid ELF32 executable (021_elf.c's own
 ...
 * task). New this chapter, so 021_semaphore.c can record which task
 ...
 * call on purpose: 021_semaphore.c needs this exact state transition
 ...
 * 021_semaphore.c's own comments for the real race this avoids). The
 ...
 * to that task. Called from 021_semaphore.c's semaphore_signal(),
...
/* Called from 021_pit.c's irq0_handler() on every single real IRQ0
